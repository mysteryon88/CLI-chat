...
int registrationCheck(const char *nik, const char *pass)
{
	...
	sql = (char*)malloc((strlen("SELECT * from Clients WHERE Nik LIKE '") + strlen(nik)) + strlen("')") * sizeof(char));
	strcpy(sql,"SELECT * from Clients WHERE Nik LIKE '");
	strcat(sql, nik);
	strcat(sql, "'");
...
int isExistFriend(const char *table, const char *nik)
{
	char *zErrMsg = 0;
	char *sql;
	sql = (char*)malloc((strlen("SELECT EXISTS(SELECT Friends FROM ") + strlen(table) + strlen(" WHERE Friends LIKE '") + strlen(nik) + strlen("')")) * sizeof(char));
	strcpy(sql, "SELECT EXISTS(SELECT Friends FROM ");
	strcat(sql, table);
	strcat(sql, " WHERE Friends LIKE '");
	strcat(sql, nik);
	strcat(sql, "')");
